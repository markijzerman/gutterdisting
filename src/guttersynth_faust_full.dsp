...
// Filter preset selector (0-4)
// 0 = Growling (30-308 Hz), 1 = Harmonic (64-360 Hz), 2 = Dense (68-658 Hz), 3 = Standard (97-720 Hz), 4 = MIDI-based
filterPreset = nentry("h:[1]Filters/[2]filter preset", 0, 0, 4, 1);

// Root note for MIDI mode (preset 4)
rootNote = hslider("h:[1]Filters/[3]root (MIDI)", 33, 12, 72, 1);

// Filter intervals for MIDI mode (semitones from each voice's note)
fInt1 = hslider("h:[1]Filters/[4]interval 1", 0, -24, 24, 1);
fInt2 = hslider("h:[1]Filters/[5]interval 2", 5, -24, 24, 1);
fInt3 = hslider("h:[1]Filters/[6]interval 3", 12, -24, 24, 1);
fInt4 = hslider("h:[1]Filters/[7]interval 4", 17, -24, 24, 1);
fInt5 = hslider("h:[1]Filters/[8]interval 5", 24, -24, 36, 1);
fInt6 = hslider("h:[1]Filters/[9]interval 6", 29, -24, 36, 1);
fInt7 = hslider("h:[1]Filters/[10]interval 7", 36, -24, 48, 1);
fInt8 = hslider("h:[1]Filters/[11]interval 8", 41, -24, 48, 1);

// Random frequency offset (cents, applied differently to each filter)
freqSpread = hslider("h:[1]Filters/[12]freq spread", 0, 0, 100, 1);
...
// Each preset has 8 frequencies in Hz
//==============================================================================

// Preset 0: Growling - very low fundamentals for growling bass character
// From filters.txt line 8: 30 60 90 122 166 201 270 293 308...
growlFreq(0) = 30;   growlFreq(1) = 60;   growlFreq(2) = 90;   growlFreq(3) = 122;
growlFreq(4) = 166;  growlFreq(5) = 201;  growlFreq(6) = 270;  growlFreq(7) = 308;

// Preset 1: Harmonic - evenly spaced harmonic series for organ-like tones
// From filters.txt line 9: 64 98 130 163 196 227 294 327 360...
harmFreq(0) = 64;   harmFreq(1) = 98;   harmFreq(2) = 130;  harmFreq(3) = 163;
harmFreq(4) = 196;  harmFreq(5) = 227;  harmFreq(6) = 294;  harmFreq(7) = 360;

// Preset 2: Dense - many close frequencies for rich timbre
// From filters.txt line 6: 68 97 170 248 391 449 531 589 658...
denseFreq(0) = 68;   denseFreq(1) = 97;   denseFreq(2) = 170;  denseFreq(3) = 248;
denseFreq(4) = 391;  denseFreq(5) = 449;  denseFreq(6) = 531;  denseFreq(7) = 658;

// Preset 3: Standard - mid-range with good spread
// From filters.txt line 1: 97 156 186 200 243 318 333 383 435...
stdFreq(0) = 97;   stdFreq(1) = 156;  stdFreq(2) = 200;  stdFreq(3) = 243;
stdFreq(4) = 318;  stdFreq(5) = 383;  stdFreq(6) = 495;  stdFreq(7) = 720;

// Select frequency for a given preset and filter index
// Voice offset multiplier for variety between voices (1.0 to 1.25)
voiceFreqMult(v) = 1.0 + (v * 0.035);

// Get base frequency for preset and filter index
presetFreq(preset, idx) = ba.selectn(5, preset,
    growlFreq(idx),
    harmFreq(idx),
    denseFreq(idx),
    stdFreq(idx),
    mtof(rootNote + voiceNote(0) + midiInterval(idx))  // MIDI mode uses intervals
) with {
    midiInterval(0) = fInt1; midiInterval(1) = fInt2; midiInterval(2) = fInt3; midiInterval(3) = fInt4;
    midiInterval(4) = fInt5; midiInterval(5) = fInt6; midiInterval(6) = fInt7; midiInterval(7) = fInt8;
};
...
with {
    ...
    seed = (voiceIdx * 8 + filterIdx) * 7919;  // prime number for variety
...
// For preset modes (0-3): use Hz presets with voice multiplier for variety
// For MIDI mode (4): use MIDI-based calculation with voice offsets
getFilterFreq(n, i) = ba.if(filterPreset < 4,
    // Preset mode: use Hz values with voice multiplier for variety
    presetFreq(filterPreset, i) * voiceFreqMult(n) * centsToRatio(spreadOffset(n, i)),
    // MIDI mode: use intervals relative to voice note
    mtof(rootNote + voiceNoteOffset(n) + midiInt(i)) * centsToRatio(spreadOffset(n, i))
) with {
    voiceNoteOffset(0) = voice1Note; voiceNoteOffset(1) = voice2Note;
    voiceNoteOffset(2) = voice3Note; voiceNoteOffset(3) = voice4Note;
    voiceNoteOffset(4) = voice5Note; voiceNoteOffset(5) = voice6Note;
    voiceNoteOffset(6) = voice7Note; voiceNoteOffset(7) = voice8Note;
    midiInt(0) = fInt1; midiInt(1) = fInt2; midiInt(2) = fInt3; midiInt(3) = fInt4;
    midiInt(4) = fInt5; midiInt(5) = fInt6; midiInt(6) = fInt7; midiInt(7) = fInt8;
};

// Filter bank for voice n (8 filters with preset-based or MIDI-based frequencies)
filterBank(n) = _ <: (
    biquadBP(getFilterFreq(n, 0), filterQWithDrift),
    biquadBP(getFilterFreq(n, 1), filterQWithDrift),
    biquadBP(getFilterFreq(n, 2), filterQWithDrift),
    biquadBP(getFilterFreq(n, 3), filterQWithDrift),
    biquadBP(getFilterFreq(n, 4), filterQWithDrift),
    biquadBP(getFilterFreq(n, 5), filterQWithDrift),
    biquadBP(getFilterFreq(n, 6), filterQWithDrift),
    biquadBP(getFilterFreq(n, 7), filterQWithDrift)
) :> _;
